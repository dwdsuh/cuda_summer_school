...
static void conv2d(Tensor *in_t, Tensor *out_t, Tensor *weight_t,
  ...
  int n_thread = C_OUT * H_OUT * W_OUT;
  dim3 blockDim(512);
...
static void instancenorm2d(Tensor *in_t, Tensor *out_t, Tensor *weight_t,
  ...
  dim3 blockDim(512);
  dim3 gridDim((C + 512 -1) / 512);

  instancenorm2d_kernel<<<gridDim, blockDim>>>(in, out, weight, bias, C, H, W);
}
...
static void linear(Tensor *in_t, Tensor *out_t, Tensor *weight_t,
  ...
  int n_thread = N * H_OUT;
  dim3 blockDim(512);
  dim3 gridDim((n_thread + 512 - 1) / 512);
  linear_kernel<<<gridDim, blockDim>>>(in, out, weight, bias, H_IN, H_OUT, N);
}

__global__ void maxpool2d_kernel(float *in, float *out, 
	int H_IN, int W_IN, int H_OUT, int W_OUT, int N, int kH, int kW){
  int tidx = blockDim.x * blockIdx.x + threadIdx.x;

  int n = tidx / (H_OUT * W_OUT);
  int h_out = (tidx / W_OUT) % H_OUT;
  int w_out = tidx % W_OUT;
...
static void maxpool2d(Tensor *in_t, Tensor *out_t, int kH, int kW) {
  ...
  int N = in_t->shape[0];
  int n_thread = N * H_OUT * W_OUT;
  dim3 blockDim(512);
  dim3 gridDim((n_thread + 512 -1) / 512);
  maxpool2d_kernel<<<gridDim, blockDim>>>(in, out, H_IN, W_IN, H_OUT, W_OUT, N, kH, kW);
}
